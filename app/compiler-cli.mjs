...
if (args[0] === 'compile') {
    ...
    if (!filename) {
        console.error("Usage: node app/compiler-cli.mjs compile <file>");
        process.exit(1);
    }
    ...
    if (!bytecode) {
        console.error("Usage: node app/compiler-cli.mjs disassemble <bytecode>");
        process.exit(1);
    }
    const compiler = new Compiler();
    ...
    console.log("Usage: node app/compiler-cli.mjs <command> [args]");
